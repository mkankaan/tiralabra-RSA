...
- **Key** - Represents both a public and private key. Implemented using Python's built-in "named tuple" data structure, since the class only needs to store data and doesn't have any methods.

Modules:

- **key_generator.py** - Contains a function for generating a public and private key as a pair of Key objects. Relies on *prime_generator.py* to find primes for the keys. Also contains the modular multiplicative inverse algorithm, which is used to calculate the private exponent.
...
- **miller_rabin.py** - Contains the Miller-Rabin algorithm and a helper function for deconstructing an integer into powers of two. I split this into a separate module from the rest of the prime generation 
...
No tools were used to measure performance because it wouldn't really have given any useful information. There isn't really any reference point for how many numbers have to be tested before a prime is found, which is the slowest and computationally heaviest part of the program and relies almost completely on RNG.
